...
class Settings extends Component
{
    ...
    if ($this->logo != null) {
            ...
            // borra el logo anterior si es que hay         
        if (File::exists(public_path($tempLogo))) {            
          File::delete($tempLogo);
      }           

      // guarda logo en customfilename
      $customFileName = uniqid() . '_.' . $this->logo->extension();            
      $config->logo = $customFileName;
      $config->save();

      // guarda logo en directorio public
      $this->logo->storeAs('', $customFileName, 'public2');  //CONFIGURAR DRIVER PUBLIC2 EN FILESYSTEM     
        
      // desplega el nuevo logo
      $this->logoPreview = $customFileName;   

      // limpia logo precargado
      $this->logo=null;
